...
#    #target=0,1的樹
    ...
    #target=1,2的樹
    if(tr<50):
        featuretree = feature0[49:149,] 
        targettree = target0[49:149,]
    else:
        featuretree = feature0[50:149,]
        targettree = target0[50:149,]
...
#       
        #target0,1跑出來的model
    node = dict()
    node['data'] = range(len(targettree))
    Tree = [];
    Tree.append(node)
    t = 0
    while(t<len(Tree)):
        idx = Tree[t]['data']
        if(sum(targettree[idx])==0):
            Tree[t]['leaf']=1
            Tree[t]['decision']=0
        elif(sum(targettree[idx])==len(idx)):
            Tree[t]['leaf']=1
            Tree[t]['decision']=1
        else:
            bestIG = 0
            for i in range(featuretree.shape[1]):
                pool = list(set(featuretree[idx,i]))
                for j in range(len(pool)-1):
                    thres = (pool[j]+pool[j+1])/2
                    G1 = []
                    G2 = []
                    for k in idx:
                        if(featuretree[k,i]<=thres):
                            G1.append(k)
                        else:
                            G2.append(k)
                    thisIG = IG(sum(targettree[G1]==1),sum(targettree[G1]==0),sum(targettree[G2]==1),sum(targettree[G2]==0))
                    if(thisIG>bestIG):
                        bestIG = thisIG
                        bestG1 = G1
                        bestG2 = G2
            ...
            if(bestIG>0):
                ...
                Tree[t]['leaf']=1
                if(sum(targettree[idx]==1)>sum(targettree[idx]==0)):
                    Tree[t]['decision']=1
                else:
                    Tree[t]['decision']=0
        t+=1
