...
for i in range(len(tradeday)):
    date = tradeday[i]
    idx = np.nonzero(TAIEX[:,0]//10000==date)[0] #拿出index col=0 那天整天資料的每一分鐘(若index=0)一天約有300多筆
    idx.sort()
    #想設停損為30點############################
    p1=TAIEX[idx[0],2] ##開盤價
    idx2 = np.nonzero(TAIEX[idx,4]>=p1+30)[0] #漲30是我的lose 停損
    idx3 = np.nonzero(TAIEX[idx,3]<=p1-30)[0] #跌30是我的profit 停利
